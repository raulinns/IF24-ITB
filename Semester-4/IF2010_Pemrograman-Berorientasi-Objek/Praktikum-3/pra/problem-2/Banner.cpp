...
std::string Banner::getName() const { return name; }

int Banner::getExpiryDate() const { return expiryDate; }

int Banner::getItemCount() const { return itemCount; }
...
void Banner::pullItem(int currentDate) {
     ...
     *
     * Saat `validateExpiryDate()` atau `validateItemAvailability()` menemui
     * error lalu menjalankan instruksi `throw`, eksekusi program di dalam
     * fungsi `pullItem()` ini akan SEKETIKA BERHENTI
     *
