...
int main() {
    ...
    while (cin >> command) {
        try {
            if (command == "CREATE") {
                ...
                cin >> type >> name >> cap;
                if (type == "STRING")
                    stringBays[name] = new CargoBay<string>(cap);
                else if (type == "INT")
                    intBays[name] = new CargoBay<int>(cap);
                ...
                cin >> type >> name;
                if (type == "STRING")
                    Formatter::printItem("Popped from " + name,
                                         stringBays[name]->pop());
                else if (type == "INT")
                    Formatter::printItem("Popped from " + name,
                                         intBays[name]->pop());
                ...
                if (type == "STRING") {
                    string target;
                    cin >> target;
                    Formatter::printItem("Extracted from " + name,
                                         stringBays[name]->extractItem(target));
                } else if (type == "INT") {
                    int target;
                    cin >> target;
                    Formatter::printItem("Extracted from " + name,
                                         intBays[name]->extractItem(target));
                }
                ...
                if (type == "STRING")
                    stringBays[src]->transferTo(*stringBays[dest], count);
                else if (type == "INT")
                    intBays[src]->transferTo(*intBays[dest], count);
                Formatter::printMessage("Transfer " + src + " to " + dest +
                                        " success");
            }
    ...
    for (auto &pair : stringBays)
        delete pair.second;
    for (auto &pair : intBays)
        delete pair.second;
