...
Player::Player(std::string name, int initialGems)
    : name(name), gems(initialGems) {}

std::string Player::getName() const { return name; }

int Player::getGems() const { return gems; }
...
void Player::deductGems(int amount) {
    ...
    // 2. Jika tidak cukup, langsung lemparkan (throw)
